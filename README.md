...
<head>
    ...
    <title>資安監控與氣象風險綜合儀表板 v3.7 (訪客鎖定版)</title>
    <!-- 引入 Tailwind CSS -->
    ...
    <script type="module">
            ...
            loadUserSettings().then(settings => {
                ...
                if (!mapInstance) {
                    if (savedCity) {
                        ...
                        updateWeather("台北市", 25.0330, 121.5654);
                    }
                }
            });
        }
            ...
            try {
                ...
                if (docSnap.exists()) {
                    ...
                    if (data.username) {
                        ...
                        document.getElementById('user-uid-display').textContent = data.username; // 同步 Header
                    }
                    if (data.bio) {
        ...
        async function saveUserSettings() {
            ...
            const username = document.getElementById('setting-username').value.trim(); // [新增]
            const bio = document.getElementById('setting-bio').value.trim(); // [新增]
            ...
            const settingsData = { defaultCity: city, username: username, bio: bio }; // [新增]
            if (tempAvatarBase64) {
            ...
            try {
                if (!currentUser.uid.startsWith('guest-offline')) {
                    ...
                    if (username) {
                        document.getElementById('user-uid-display').textContent = username; // [新增] 更新 Header
                    }
                    
                    alert("設定已儲存！");
                } else {
                    alert("訪客模式無法雲端儲存，但本次操作有效。");
        ...
        function toggleAuthMode() {
            ...
            document.getElementById('auth-title').textContent = isRegisterMode ? "註冊新帳號" : "系統登入";
            document.getElementById('auth-submit-btn').textContent = isRegisterMode ? "註冊" : "登入";
            document.getElementById('auth-toggle-btn').textContent = isRegisterMode ? "已有帳號？登入" : "建立新帳號";
        ...
        async function handleAuthSubmit() {
            ...
            if (!email || !password) { errorMsg.textContent = "請輸入 Email 和密碼"; return; }
            errorMsg.textContent = "處理中...";
            try {
                if (isRegisterMode) await createUserWithEmailAndPassword(auth, email, password);
                else await signInWithEmailAndPassword(auth, email, password);
            } catch (error) { errorMsg.textContent = "登入/註冊失敗: " + error.code; }
        }
        ...
        async function updateWeather(cityName, lat, lon) {
            ...
            try {
                ...
                document.getElementById('display-temp').textContent = `${current.temperature_2m}°C`;
                ...
                document.getElementById('display-apparent').textContent = `${current.apparent_temperature}°C`;
            ...
            if (isHeavyRain || isStrongWind) {
                alertStatusDiv.innerHTML = `
                    <div class="bg-red-500/10 border border-red-500 rounded-2xl p-8 text-center animate-fade-in">
                        ...
                        <h3 class="text-2xl font-bold text-red-500 mb-2">災害預警發布中</h3>
                        <p class="text-gray-400 mb-6">監測地點：${city}</p>
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4 text-left max-w-2xl mx-auto">
                            ${isHeavyRain ? `<div class="bg-red-500/20 p-4 rounded-xl flex items-center"><i class="fas fa-cloud-showers-heavy text-red-400 text-xl mr-3"></i><div><p class="font-bold text-red-300">豪大雨警報</p><p class="text-xs text-red-200/70">降雨強度偏高，請注意淹水風險。</p></div></div>` : ''}
                            ${isStrongWind ? `<div class="bg-orange-500/20 p-4 rounded-xl flex items-center"><i class="fas fa-wind text-orange-400 text-xl mr-3"></i><div><p class="font-bold text-orange-300">強風警報</p><p class="text-xs text-orange-200/70">風速過大，請加固戶外設施。</p></div></div>` : ''}
                        </div>
                    </div>
                `;
            } else {
                alertStatusDiv.innerHTML = `
                    <div class="bg-green-500/10 border border-green-500/30 rounded-2xl p-12 text-center animate-fade-in">
                        ...
                        <h3 class="text-2xl font-bold text-green-500 mb-2">目前無災害警報</h3>
                        <p class="text-gray-400">監測地點：${city} - 各項數值均在安全範圍內。</p>
                    </div>
                `;
            }
        }
        ...
        function renderTrendChart(hourlyData) {
            ...
            trendChartInstance = new Chart(ctx, {
                ...
                data: {
                    labels: labels,
                    datasets: [
                        { label: '降雨機率 (%)', data: rainProbs, backgroundColor: 'rgba(59, 130, 246, 0.3)', yAxisID: 'y1', borderRadius: 4 },
                        { label: '氣溫 (°C)', data: temps, type: 'line', borderColor: '#fbbf24', backgroundColor: 'rgba(251, 191, 36, 0.1)', yAxisID: 'y', fill: true, tension: 0.4 }
                    ]
                },
                options: {
        ...
        function getWeatherDesc(code) {
            const codes = { 0: "晴朗", 1: "主要晴朗", 2: "多雲", 3: "陰天", 45: "霧", 48: "沉積霧", 51: "細雨", 61: "微雨", 71: "小雪", 80: "陣雨", 95: "雷雨" };
            return codes[code] || "未知狀況";
        }
        ...
        async function generateRiskReport(city, data) {
            ...
            // 優化的 AI Prompt (生活建議)
            const prompt = `你是一位貼心的氣象生活助理。請針對以下實時氣象數據，為使用者生成一份生活建議報告。

**氣象數據：**
* 城市：${city}
* 氣溫：${data.temp}°C (體感：${data.apparent}°C)
* 濕度：${data.humidity}%
* 降雨量：${data.rain} mm
* 風速：${data.wind} km/h
* 能見度：${data.visibility} km
* 天氣狀況：${data.desc}

**報告要求（請使用 Markdown 格式）：**

1.  🌤️ **天氣綜述**：用一句話形容現在的體感舒適度。
2.  👕 **穿搭建議**：建議穿著什麼類型的衣物（例如：透氣、保暖、攜帶雨具）。
3.  🏃 **活動建議**：適合戶外運動還是室內活動？有無特定注意事項。
4.  🏥 **健康叮嚀**：針對溫度、濕度或風速的健康提醒（例如：多喝水、注意防曬、小心路滑）。
...
請保持語氣親切、實用。`;
            ...
            try {
                ...
                if (text) {
                    ...
                    // 根據天氣狀況更新風險標籤 (這裡指天氣影響程度)
                    const riskLevel = (data.rain > 10 || data.wind > 40 || data.temp > 35) ? "不適宜戶外" : (data.rain > 2 || data.humidity > 85 ? "需注意" : "舒適");
                    const levelEl = document.getElementById('risk-level-tag');
                    if (levelEl) {
                        levelEl.textContent = riskLevel;
                        let colorClass = 'bg-green-500';
                        if (riskLevel === '不適宜戶外') colorClass = 'bg-red-500 pulse-red';
                        else if (riskLevel === '需注意') colorClass = 'bg-yellow-500';
                ...
                console.error(e); 
                reportEl.innerHTML = "報告分析暫時無法使用，請檢查 API 連線。"; 
            }
        }
        ...
        function initMap(lat, lon) {
            ...
            userMarker = L.marker([lat, lon]).addTo(mapInstance).bindPopup("監控點").openPopup();
        }
        ...
        function updateUserUI() {
            if(currentUser) {
                const type = currentUser.isAnonymous ? "訪客" : "會員";
                // 這裡會先顯示 UID，稍後 loadUserSettings 會更新為姓名
    ...
    <header class="sticky top-0 z-50 glass-morphism border-b border-white/10 px-6 py-4">
        <div class="container mx-auto flex justify-between items-center">
            <div class="flex items-center space-x-3">
                ...
                <div>
                    <h1 class="text-xl font-bold">以風暴預測未來,以暗網守護天空<span class="text-blue-500 text-sm">PRO</span></h1>
            ...
            <div id="main-nav-buttons" class="hidden-section hidden md:flex items-center space-x-8 mx-4">
                <button id="nav-dashboard-btn" class="nav-btn text-sm font-medium text-gray-400 hover:text-white transition-all pb-1 active">儀錶板</button>
                <button id="nav-alert-btn" class="nav-btn text-sm font-medium text-gray-400 hover:text-red-400 transition-all pb-1">災害預警</button>
                <button id="nav-settings-btn" class="nav-btn text-sm font-medium text-gray-400 hover:text-blue-400 transition-all pb-1">個人化設定</button>
            </div>
        ...
        <div id="auth-view" class="flex-1 flex items-center justify-center min-h-[500px]">
            <div class="w-full max-w-md bg-white dark:bg-cardBg p-8 rounded-2xl shadow-2xl border border-gray-200 dark:border-white/5 relative overflow-hidden">
                ...
                <div class="relative z-10 text-center">
                    ...
                    <h2 id="auth-title" class="text-2xl font-bold mb-2">系統登入</h2>
                    <p class="text-gray-500 text-sm mb-8">存取即時資安與氣象風險數據</p>
                    <div class="space-y-4 text-left">
                        ...
                        <div>
                            <label class="block text-xs font-medium text-gray-400 mb-1 ml-1">密碼</label>
                            <input type="password" id="auth-password" class="w-full bg-gray-50 dark:bg-black/20 border border-gray-200 dark:border-white/10 rounded-xl px-4 py-3 outline-none focus:border-blue-500 transition-all" placeholder="••••••••">
                        </div>
                        <p id="auth-error" class="text-red-500 text-xs font-bold h-4"></p>
                        <button id="auth-submit-btn" class="w-full bg-blue-600 hover:bg-blue-500 text-white font-bold py-3 rounded-xl shadow-lg shadow-blue-600/30 transition-all">登入</button>
                    </div>
                    <div class="mt-6 flex flex-col space-y-3">
                        <button id="auth-toggle-btn" class="text-sm text-gray-500 hover:text-blue-500 transition-colors">建立新帳號</button>
                        <button id="guest-login-btn" class="text-xs font-bold text-gray-400 hover:text-gray-200 border border-gray-200 dark:border-white/10 rounded-lg py-2 hover:bg-white/5 transition-all">以訪客身份試用 <i class="fas fa-arrow-right ml-1"></i></button>
                    </div>
                </div>
            </div>
        </div>
            ...
            <div id="dashboard-view" class="space-y-8 animate-fade-in">
                <section class="grid grid-cols-1 lg:grid-cols-4 gap-6">
                    <div class="lg:col-span-1 weather-card rounded-2xl p-6 flex flex-col justify-between">
                        <div>
                            <h3 class="text-white font-bold mb-4 flex items-center"><i class="fas fa-search-location mr-2 text-blue-400"></i> 地點監測</h3>
                            <div class="space-y-4">
                                <input type="text" id="weather-search-input" placeholder="輸入城市 (如: 台北)" class="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-3 text-sm text-white focus:ring-2 focus:ring-blue-500 outline-none">
                                <button id="weather-search-btn" class="w-full bg-blue-600 hover:bg-blue-500 text-white font-bold py-3 rounded-xl transition-all">更新分析</button>
                            </div>
                        </div>
                    ...
                    <div id="weather-info-container" class="lg:col-span-3 bg-white dark:bg-cardBg rounded-2xl p-8 border border-gray-200 dark:border-white/5 shadow-xl">
                        <div class="flex flex-wrap items-center justify-between mb-8">
                            <div class="flex items-center space-x-6">
                                ...
                                <div>
                                    <h2 id="display-city" class="text-4xl font-black mb-1">台北市</h2>
                                    <p id="display-desc" class="text-gray-500 font-medium italic">偵測中...</p>
                                </div>
                            </div>
                            <div class="text-right">
                                <p class="text-xs text-gray-400 uppercase mb-1">即時氣溫</p>
                                <span id="display-temp" class="text-5xl font-black text-blue-500 tracking-tighter">--°C</span>
                            </div>
                        </div>
                        <div class="grid grid-cols-2 md:grid-cols-5 gap-6 text-center mb-8">
                            <div class="bg-gray-100 dark:bg-white/5 p-4 rounded-xl"><p class="text-[10px] text-gray-400 mb-1">體感</p><span id="display-apparent" class="font-bold">--°C</span></div>
                            <div class="bg-gray-100 dark:bg-white/5 p-4 rounded-xl"><p class="text-[10px] text-gray-400 mb-1">濕度</p><span id="display-humidity" class="font-bold">--%</span></div>
                            <div class="bg-gray-100 dark:bg-white/5 p-4 rounded-xl"><p class="text-[10px] text-gray-400 mb-1">降雨量</p><span id="display-rain" class="font-bold">-- mm</span></div>
                            <div class="bg-gray-100 dark:bg-white/5 p-4 rounded-xl"><p class="text-[10px] text-gray-400 mb-1">風速</p><span id="display-wind" class="font-bold">-- km/h</span></div>
                            <div class="bg-gray-100 dark:bg-white/5 p-4 rounded-xl"><p class="text-[10px] text-gray-400 mb-1">能見度</p><span id="display-visibility" class="font-bold">-- km</span></div>
                        </div>
                        <div class="border-t border-gray-200 dark:border-white/5 pt-6">
                            <div class="flex justify-between items-end mb-4">
                                <div><h3 class="font-bold text-gray-700 dark:text-gray-200"><i class="fas fa-chart-line mr-2 text-primary"></i>未來 24 小時趨勢</h3><p class="text-xs text-gray-500 dark:text-gray-400">Temperature & Precipitation Forecast</p></div>
                            </div>
                ...
                <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
                    <div class="lg:col-span-1 bg-white dark:bg-cardBg p-6 rounded-2xl shadow-xl h-[550px] flex flex-col border border-white/5">
                        <div class="flex justify-between items-center mb-6">
                            <h3 class="text-lg font-bold flex items-center"><i class="fas fa-robot mr-2 text-blue-500"></i> AI 生活氣象助理</h3>
                            <div id="risk-level-tag" class="px-3 py-1 rounded-full text-xs font-bold bg-gray-500 text-white">等待中</div>
                        </div>
                        <div id="risk-report-content" class="flex-1 overflow-y-auto prose dark:prose-invert prose-sm scrollbar-thin"><p class="text-gray-500 italic">正在運用 Groq AI 分析氣象並提供生活建議...</p></div>
                    </div>
                    <div class="lg:col-span-2 bg-white dark:bg-cardBg p-6 rounded-2xl shadow-xl h-[550px] flex flex-col border border-white/5">
                        <div class="flex justify-between items-center mb-4"><h3 class="text-lg font-bold"><i class="fas fa-map-marked-alt mr-2 text-indigo-500"></i> 監控地理節點</h3></div>
            ...
            <div id="alert-view" class="hidden-section w-full h-full flex items-start justify-center pt-10 animate-fade-in">
                <div class="w-full max-w-4xl">
                    <div class="flex items-center justify-between mb-8">
                        <h2 class="text-2xl font-bold text-gray-800 dark:text-white flex items-center"><i class="fas fa-broadcast-tower text-red-500 mr-3"></i> 災害預警中心</h2>
                        <span class="text-sm text-gray-500">即時監控：溫度、風速、降雨量</span>
                    </div>
                    
                    <div id="alert-status-container">
                        <!-- 這裡會由 JS 動態插入內容 -->
                        <div class="bg-gray-100 dark:bg-white/5 rounded-2xl p-12 text-center">
                            <p class="text-gray-500">正在讀取氣象數據...</p>
                        </div>
                    </div>

                    <!-- 測試控制區 -->
                    <div class="mt-6 flex justify-center space-x-4">
                        <button id="test-normal-btn" class="px-4 py-2 bg-green-500/10 text-green-500 hover:bg-green-500 hover:text-white rounded-lg transition-all text-xs font-bold border border-green-500/30">模擬正常</button>
                        <button id="test-rain-btn" class="px-4 py-2 bg-blue-500/10 text-blue-500 hover:bg-blue-500 hover:text-white rounded-lg transition-all text-xs font-bold border border-blue-500/30">模擬豪雨</button>
                        <button id="test-wind-btn" class="px-4 py-2 bg-orange-500/10 text-orange-500 hover:bg-orange-500 hover:text-white rounded-lg transition-all text-xs font-bold border border-orange-500/30">模擬強風</button>
                        <button id="test-danger-btn" class="px-4 py-2 bg-red-500/10 text-red-500 hover:bg-red-500 hover:text-white rounded-lg transition-all text-xs font-bold border border-red-500/30">模擬複合災害</button>
                    </div>

                    <div class="mt-8 grid grid-cols-1 md:grid-cols-3 gap-6">
                        <div class="bg-white dark:bg-cardBg p-6 rounded-xl border border-gray-200 dark:border-white/5 shadow-lg">
                            <h4 class="font-bold mb-2 text-gray-700 dark:text-gray-200">防範豪雨</h4>
                            <p class="text-sm text-gray-500">檢查機房排水系統，確保低窪地區設備架高。</p>
                        </div>
                        <div class="bg-white dark:bg-cardBg p-6 rounded-xl border border-gray-200 dark:border-white/5 shadow-lg">
                            <h4 class="font-bold mb-2 text-gray-700 dark:text-gray-200">防範強風</h4>
                            <p class="text-sm text-gray-500">加固戶外天線與通訊塔，檢查備用電源。</p>
                        </div>
                        <div class="bg-white dark:bg-cardBg p-6 rounded-xl border border-gray-200 dark:border-white/5 shadow-lg">
                            <h4 class="font-bold mb-2 text-gray-700 dark:text-gray-200">緊急聯絡</h4>
                            <p class="text-sm text-gray-500">若發生災害，請立即通報資安應變中心 (SOC)。</p>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div id="settings-view" class="hidden-section w-full h-full flex items-start justify-center pt-10 animate-fade-in">
                <div class="w-full max-w-2xl bg-white dark:bg-cardBg rounded-2xl p-8 shadow-2xl border border-gray-200 dark:border-white/5">
                    <div class="border-b border-gray-200 dark:border-white/10 pb-6 mb-6">
                        <h2 class="text-2xl font-bold flex items-center"><i class="fas fa-sliders-h mr-3 text-blue-500"></i> 個人化介面設定</h2>
                        <p class="text-gray-500 text-sm mt-1">調整您的儀表板預設值與顯示偏好</p>
                    </div>
                    ...
                    <div class="space-y-8">
                        ...
                        <div class="flex items-center space-x-6">
                            ...
                            <div>
                                <h4 class="font-bold text-lg">更換頭像</h4>
                                <p class="text-sm text-gray-500 mb-2">支援 JPG, PNG (最大 1MB)</p>
                                <p class="text-xs text-blue-400">點擊相機圖示選擇圖片</p>
                            </div>
                        </div>

                        <div>
                            <label class="block text-sm font-medium text-gray-400 mb-2">預設監控城市</label>
                            <div class="flex">
                                ...
                                <input type="text" id="setting-default-city" class="flex-1 bg-white dark:bg-black/20 border border-gray-300 dark:border-white/10 rounded-r-xl px-4 py-3 outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500 transition-all" placeholder="例如: 台中市">
                            </div>
                            <p class="text-xs text-gray-500 mt-2"><i class="fas fa-info-circle mr-1"></i> 系統將在登入時自動載入此城市的天氣與 AI 風險分析。</p>
                        </div>
                        
                        <!-- 個人資料區塊 -->
                        <div class="pt-6 border-t border-gray-200 dark:border-white/10 space-y-4">
                            <h4 class="font-bold text-gray-700 dark:text-gray-200 mb-2">個人資料</h4>
                            
                            <div>
                                <label class="block text-sm font-medium text-gray-400 mb-1">用戶姓名</label>
                                <input type="text" id="setting-username" class="w-full bg-gray-50 dark:bg-black/20 border border-gray-200 dark:border-white/10 rounded-xl px-4 py-2 outline-none focus:border-blue-500 transition-all" placeholder="您的暱稱">
                            </div>

                            <div>
                                <label class="block text-sm font-medium text-gray-400 mb-1">自我介紹</label>
                                <textarea id="setting-bio" rows="3" class="w-full bg-gray-50 dark:bg-black/20 border border-gray-200 dark:border-white/10 rounded-xl px-4 py-2 outline-none focus:border-blue-500 transition-all resize-none" placeholder="介紹一下自己..."></textarea>
                            </div>
                        </div>
